...
if($num_sql == 1){
	//echo $bill;
	$cmd="UPDATE `settings` SET `billingip` = '$billip',`billingop` = '$billop', `reports` = '$report', `labreports` = '$lab_reports_perm', `printbill` = '$printbill', `masterentry` = '$master', `registration` = '$reg', `medication` = '$medication_perm', `complaints` = '$complaints_perm', `activitychart` = '$activechart_perm',`clinicalchart` = '$clinicalchart_perm', `investigation` = '$investigation_perm', `patientinfo` = '$info', `admission` = '$admission_perm' WHERE `settings`.`role` = $role;";
	//echo $cmd;exit;
	if(mysql_query($cmd))
	{
